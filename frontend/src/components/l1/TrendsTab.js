...
function deriveCharts(series, skillsData) {
  ...
  for (const [label, pts] of Object.entries(series || {})) {
    ...
    const vuln = Math.round(
      Math.min(100, 0.6 * Math.min(decline, 100) + 0.4 * Math.min(aiAvg * 200, 100))
    );

    if (!roleMap[role]) {
      roleMap[role] = { postings: 0, aiSum: 0, aiCount: 0, vulnSum: 0, vulnCount: 0 };
    }
  ...
  const roleComp = Object.entries(roleMap).map(([role, r]) => ({
    ...
    aiRate: parseFloat(
      (r.aiCount ? r.aiSum / r.aiCount : 0).toFixed(2)
    ),
    vulnerability: Math.round(
      r.vulnCount ? r.vulnSum / r.vulnCount : 0
    ),
  }));
  ...
  const cityData = Object.entries(cityMap).map(([city, c]) => ({
    ...
    change: parseFloat(
      (c.prevSum > 0 ? ((c.currSum - c.prevSum) / c.prevSum * 100) : 0).toFixed(1)
    ),
  }));
...
export default function TrendsTab() {
  ...
  const { roleComp, cityData, radarData } = deriveCharts(data.series, skillsData);
  ...
  const series = Object.values(dateMap)
    .map((x) => ({ date: x.date, count: x.count, ai_rate: x.n ? x.ai_rate / x.n : 0 }))
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
              <thead style={{ position: "sticky", top: 0, background: C.card }}>
                <tr style={{ borderBottom: `1px solid ${C.border}` }}>
                  {["Role", "Postings", "AI Rate", "Vuln", "Signal"].map((h) => (
                    <th
                      key={h}
                      style={{
                        padding: "5px 8px",
