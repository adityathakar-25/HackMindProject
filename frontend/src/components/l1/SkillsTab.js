...
function GapMap({ skills }) {
  const top = [...skills].sort((a, b) => (b.this_week || 0) - (a.this_week || 0)).slice(0, 14);
  const maxC = Math.max(...top.map((s) => s.this_week || 0), 1);
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      {top.map((sk) => {
        const trained = GOVT_TRAINED.includes(String(sk.skill || "").toLowerCase());
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 4 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div
            style={{ width: 10, height: 10, borderRadius: 2, background: C.teal }}
          />
          <span style={{ fontSize: 9, color: C.muted }}>PMKVY/SWAYAM trains</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div
            style={{
...
const CATS = [
  ...
  },
  {
    label: "Office",
    color: C.amber,
    keys: ["excel", "ms office", "tally", "sap", "crm", "salesforce", "accounting"],
  },
  {
    label: "Soft",
    color: C.violet,
...
export default function SkillsTab() {
  ...
  const allSkills = [
    ...new Map([...risingRaw, ...decliningRaw].map((s) => [s.skill, s])).values(),
  ];
  ...
  const bottomSkills = [...allSkills].slice().sort(
    (a, b) => (a.this_week || 0) - (b.this_week || 0),
  );

  const rising = noWeeklyChange ? topSkills.slice(0, 12) : risingRaw.slice(0, 12);
      ...
      {allSkills.length > 0 && (
        <>
          {noWeeklyChange && (
            ...
            >
              ℹ️ Only one scraper run found — week-over-week change isn’t available yet.
            </div>
          )}
              ...
              >
                <div style={{ marginBottom: 10, flexShrink: 0 }}>
                  ...
                  <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>
                    ...
                    {noWeeklyChange ? " · ranked by count" : " · week-over-week"}
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  {rising.map((sk, i) => (
                    <SkillRow
                      ...
                      color={C.teal}
                      showRank
                      govtTrained={GOVT_TRAINED.includes(String(sk.skill || "").toLowerCase())}
                    />
                  ))}
                </div>
              </div>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                  {declining.map((sk, i) => (
                    <SkillRow
                      ...
                      rank={i + 1}
                      skill={sk.skill}
                      count={(noWeeklyChange ? sk.this_week : sk.last_week) || 0}
                      maxCount={maxD}
                      pct={noWeeklyChange ? undefined : sk.pct_change}
                      color={C.accent}
                      showRank
            ...
            {showRight && (
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 12, flexShrink: 0 }}>
                  <div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: C.text }}>🎓 Skill Gap Map</div>
                    <div style={{ fontSize: 10, color: C.muted, marginTop: 2 }}>
                      Hired by market vs trained by PMKVY / SWAYAM
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <div style={{ background: C.tealDim, border: `1px solid ${C.teal}33`, borderRadius: 7, padding: "5px 10px", textAlign: "center" }}>
                      <div style={{ fontSize: 9, color: C.muted, textTransform: "uppercase", letterSpacing: "0.08em" }}>Govt Covered</div>
                      <div style={{ fontSize: 17, fontWeight: 700, color: C.teal, fontFamily: "monospace", lineHeight: 1.2 }}>{trainedSkills.length}</div>
                      <div style={{ fontSize: 9, color: C.muted }}>of top demand</div>
                    </div>
                    <div style={{ background: C.accentDim, border: `1px solid ${C.accent}33`, borderRadius: 7, padding: "5px 10px", textAlign: "center" }}>
                      <div style={{ fontSize: 9, color: C.muted, textTransform: "uppercase", letterSpacing: "0.08em" }}>Market Gap</div>
                      <div style={{ fontSize: 17, fontWeight: 700, color: C.accent, fontFamily: "monospace", lineHeight: 1.2 }}>{gapPct}%</div>
                      <div style={{ fontSize: 9, color: C.muted }}>not govt-trained</div>
                    </div>
                  </div>
                </div>
                {/* Bar list */}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, alignItems: "stretch" }}>
                {CATS.map((cat) => {
                  const present = allSkills
                    .filter((sk) => cat.keys.includes(String(sk.skill || "").toLowerCase()))
                  ...
                  return (
                    ...
                    >
                      <div style={{ fontSize: 10, color: cat.color, fontWeight: 700, marginBottom: 8 }}>
                        {cat.label}
                      </div>
                      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                        {present.map((sk) => (
                          ...
                          >
                            <span style={{ fontSize: 10, color: C.text }}>{sk.skill}</span>
                            <span style={{ fontSize: 10, color: cat.color, fontFamily: "monospace", fontWeight: 700 }}>
                              {sk.this_week || 0}
                            </span>
                          </div>
                        ))}
          ...
          <div style={{ marginTop: 2, display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <span style={{ color: C.muted, fontSize: 10 }}>
              Source: Naukri scrape · PMKVY/SWAYAM catalog · Refreshes on filter change ·{" "}
